...
def get_point_line(p1,p2,density,increase=0):
    ...
    line = get_equidistant_points(a,b,int(np.ceil(np.linalg.norm(vec)/density))+1)  # ceil, because more points is OK, while less points could be problematic (distance between points should not be larger than 1*density)
